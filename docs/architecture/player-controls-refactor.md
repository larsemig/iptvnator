...
- **Tunnel:** while a video frame is on screen, the video's **ancestor elements** (`body`, `.workspace-shell`, `.workspace-body`, `.workspace-content`) are made transparent so the native surface shows through.
...
- **Backdrop + hole** inverts it: keep the app opaque by default, subtract one rectangle. **No per-route panel work**; new routes need no CSS.
